...
#include "Tcdefs.h"
#include <iostream>// ***Ganter,
#include <stdlib.h>// ***Ganter,
#include <fstream>// ***Ganter,
#include <set>// ***Ganter,
#include <iterator> // ***Ganter,
...
// ***Ganter, added paramter for the password bar
...
// ***Ganter,
volatile int WizardMode = WIZARD_MODE_SYS_DEVICE;
...
// ***Ganter, "TRUE" is system encryption
...
BOOL bDontVerifyRescueDisk = TRUE; // ***Ganter, Always skip the "Verification"
BOOL bFirstSysEncResumeDone = FALSE;

// ***Ganter, in gpt, multi boot is not available. The old legacy boot modus is left for future work
int nMultiBoot = 1;
...
// ***Ganter, the checkboxes on the checklist page
BOOL CHECKLIST_A;
BOOL CHECKLIST_B;
BOOL CHECKLIST_C;
...
// ***Ganter, a debug_fuction that prints out all those system values in a console. For turning it on, also decomment the conso ecode in wWinMain
/*void debug_info()
{


std::cout<< "nCurPageNo " << nCurPageNo << std::endl;
std::cout<< "nLastPageNo " << nLastPageNo << std::endl;
std::cout<< "WizardMode " << WizardMode << std::endl;
std::cout<< "bHiddenOS " << bHiddenOS<< std::endl;
std::cout<< "bDirectSysEncModeCommand " << bDirectSysEncModeCommand << std::endl;
std::cout<< "DirectCreationMode " << DirectCreationMode << std::endl;
std::cout<< "bSystemEncryptionInProgress " << bSystemEncryptionInProgress << std::endl;
std::cout<< "bWholeSysDrive " << bWholeSysDrive<< std::endl;
std::cout<< " bSystemEncryptionStatusChanged " <<  bSystemEncryptionStatusChanged << std::endl;
std::cout<< "bSysEncDriveAnalysisInProgress " << bSysEncDriveAnalysisInProgress << std::endl;
std::cout<< "bSysEncDriveAnalysisTimeOutOccurred " <<bSysEncDriveAnalysisTimeOutOccurred<< std::endl;
std::cout<< "SysEncDetectHiddenSectors " << SysEncDetectHiddenSectors<< std::endl;
std::cout<< "SysEncDriveAnalysisStart " << SysEncDriveAnalysisStart<< std::endl;
std::cout<< "bDontVerifyRescueDisk " << bDontVerifyRescueDisk<< std::endl;
std::cout<< "bFirstSysEncResumeDone " << bFirstSysEncResumeDone << std::endl;
std::cout<< "nMultiBoott " << nMultiBoot<< std::endl;
std::cout<< "bHiddenVol " << bHiddenVol<< std::endl;
std::cout<< "bHiddenVolHost " << bHiddenVolHost<< std::endl;
std::cout<< "bHiddenVolDirect " << bHiddenVolDirect<< std::endl;
std::cout<< "bHiddenVolFinished " << bHiddenVolFinished<< std::endl;
std::cout<< "bDeviceWipeInProgress " << bDeviceWipeInProgress<< std::endl;
std::cout<< "bVolTransformThreadRunning " << bVolTransformThreadRunning<< std::endl;
std::cout<< "bVolTransformThreadToRun " << bVolTransformThreadToRun<< std::endl;
std::cout<< "bDevice " << bDevice << std::endl;
std::cout<< "showKeys " << showKeys<< std::endl;
std::cout<< "bWarnOuterVolSuitableFileSys " << bWarnOuterVolSuitableFileSys<< std::endl;
std::cout<< "ComServerMode " << ComServerMode<< std::endl;
std::cout<< "CmdSparseFileSwitch " <<CmdSparseFileSwitch<< std::endl;
std::cout<< "bForceOperation " << bForceOperation<< std::endl;
std::cout<< "bOperationSuccess " << bOperationSuccess<< std::endl;
std::cout<< "bGuiMode " << bGuiMode << std::endl;
std::cout<< "bDisplayPoolContents " << bDisplayPoolContents<< std::endl;
std::cout<< "bSparseFileSwitch " << bSparseFileSwitch<< std::endl;
std::cout<< "quickFormat " << quickFormat<< std::endl;
std::cout<< " NonSysInplaceEncStatus " <<  NonSysInplaceEncStatus<< std::endl;
std::cout<< "nWipeMode " <<nWipeMode<< std::endl;
std::cout<< "bSystemIsGPT " <<bSystemIsGPT<< std::endl;
std::cout<< "hash_algo " <<hash_algo<< std::endl;
std::cout<< "output " << __LINE__<< std::endl;
std::cout<< "-----------------------------"<< std::endl;
}*/
...
void ComboSelChangeEA (HWND hwndDlg)
{
	...
	else
	{
		...
		// ***Ganter,  Commented out because nobody needs to know those historical  cipher descripitons 
	...
	//	{
		...
		//} 
			//***Ganter changed, since it is deleted in 1.22 but not in 1.21
...
static void UpdateSysEncControls (void)
{
	...
	//Test pius todo: try to make the pause button visible without having to hover over it
	/*SetWindowTextW (GetDlgItem (hCurPage, IDC_PAUSE),
		GetString ("IDC_PAUSE"));*/ //TODELETE, das habe ich glaube ich zugefügt, hat aber nichts gebracht. Falls alles am ende normal lauft, dass hier loeschen
...
BOOL CALLBACK PageDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		case SYSENC_TYPE_PAGE:
			...
			SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_BOX_TITLE), GetString ("PAGE_1_TITLE"));// ***Ganter, changed title
			...
			// ***Ganter, set advanced button to not clickable (non-gpt functionallities are out of scope)
			EnableWindow (GetDlgItem (hwndDlg, IDC_ADVANCE_INTRO ), FALSE);
			...
			// ***Ganter, start loading the password dictonary into memory ("need" is just a random word for initializing the process)
			CheckWord( "need");
		...
		case PASSWORD_PAGE:
			{
				...
				// ***Ganter, Disable the verification password field at initialisation. It is enabled, when the password has at least a strength of "medium"
				...
				//SetCheckBox (hwndDlg, IDC_PIM_ENABLE, PimEnable); ***Ganter, copied to the advanced page
				...
				//SetWindowTextW (GetDlgItem (hwndDlg, IDC_BOX_HELP), str);  ***Ganter,
	...
	//########################## Insert random pool paramter here in password page in order to jump this step
			...
			//##################### cipherpage functionality here ############
			// ***Ganter, since cipher page is skiped, the default values are set here
			hash_algo = bSystemIsGPT? SHA512 : DEFAULT_HASH_ALGORITHM_BOOT;
			RandSetHashFunction (hash_algo);


			//#################### Wipe page functionality here#############
			// ***Ganter, set wipe modus per default to 1
		...
		case SYSENC_RESCUE_DISK_BURN_PAGE:
			{
				...
				// ***Ganter, send button is disabled at the beginning, since email field is empty
				EnableWindow (GetDlgItem (hwndDlg, IDC_SENDMAILBUTTON), FALSE);
				SendMessage (GetDlgItem (hwndDlg, IDC_EMAIL), EM_LIMITTEXT, MAX_PASSWORD, 0);//email length is max. 64 character long


				// ***Ganter, title name
				SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_BOX_TITLE), GetString (bDontVerifyRescueDisk ? "BACKUP_RESCUE_DISK_TITLE" : "RESCUE_DISK_RECORDING_TITLE"));
				SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_NEXT), GetString ("NEXT"));
				SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_PREV), GetString ("PREV"));

				// ***Ganter, display the same text for both gpt and mbr partition format (no difference between "burn the rescue disk on a CD" and "extract it to a USB")
				/*if (bSystemIsGPT)
				{*/
					/*StringCbPrintfW (szTmp, sizeof szTmp,
					...
					StringCbPrintfW (szTmp, sizeof szTmp,
						GetString (bDontVerifyRescueDisk ? "RESCUE_DISK_EFI_EXTRACT_INFO_NO_CHECK_NEW" : "RESCUE_DISK_EFI_EXTRACT_INFO"),
						szRescueDiskISO, "");
				...
				// ***Ganter, we activate it, but provide a message box with information on why the user cant go back
				EnableWindow (GetDlgItem (GetParent (hwndDlg), IDC_PREV), TRUE);

				// ***Ganter, nobody wants to burn an CD in 2020
				/*if (bSystemIsGPT)
				{*/
			...
			// ***Ganter, Copy the two information files files to the rescue disk folder		
			copy_txt_files_Rescue_Disk_Info();
			copy_txt_files_pretest();
			break;
		...
		case SYSENC_WIPE_MODE_PAGE:
		case NONSYS_INPLACE_ENC_WIPE_MODE_PAGE:// ***Ganter, now its the Checklist page
			{


		//***Ganter, changed commend out all wipe mode functionality and 
			...
			// ***Ganter, set all checkboxes to false at the beginning
			CHECKLIST_A = FALSE;
			CHECKLIST_B = FALSE;
			CHECKLIST_C = FALSE;
			...
			// ***Ganter, In case the user goes back and reloads this page, the checkboxes are empty
			//SendMessage(hWndOfCheckbox, BM_SETCHECK, BST_UNCHECKED, 0);

			SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_BOX_TITLE),L"Checklist - Before clicking 'Next'");

			// ***Ganter, copies the path of the rescue disk into the text
					wchar_t szTmp[8192];
					StringCbPrintfW (szTmp, sizeof szTmp,
					GetString ( "RESCUE_DISK_CHECKLIST"),
					szRescueDiskISO, "");
			...
			/*if (nWipeMode == TC_WIPE_1_RAND)
				...
				SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_PREV), GetString ("PREV"));
				EnableWindow (GetDlgItem (GetParent (hwndDlg), IDC_PREV), TRUE);
				EnableWindow (GetDlgItem (GetParent (hwndDlg), IDC_NEXT), FALSE);// ***Ganter, set initially to false
		...
		case SYSENC_ENCRYPTION_PAGE:
			...
			if (CreateSysEncMutex ())
			{
				...
				// ***Ganter, more information should be hidden
				...
				/*SetWindowTextW (GetDlgItem (hwndDlg, IDC_PAUSE),
					GetString ( "IDC_PAUSE"));
				EnableWindow (GetDlgItem (hwndDlg, IDC_PAUSE), TRUE);*/ //TODELETE, pause button still not visible. Falls es am ende spinnt, das hier wieder anmachen
				...
				//ToHyperlink (hwndDlg, IDC_MORE_INFO_SYS_ENCRYPTION);  ***Ganter, hyperling removed
	...
	case WM_CTLCOLORSTATIC:
		{
			...
			// ***Ganter, the color of the password meter is set here
			if ((HWND)lParam == GetDlgItem(hwndDlg, PASSWORTMETER) && pw_strength == 1) 
			{
			...
			if ((HWND)lParam == GetDlgItem(hwndDlg, PASSWORTMETER) && pw_strength == 2) 
			{
				// we're about to draw the static
				// set the text colour in (HDC)lParam
				SetBkMode((HDC)wParam,TRANSPARENT);
				...
				if ((HWND)lParam == GetDlgItem(hwndDlg, PASSWORTMETER) && pw_strength == 3) 
			{
				...
				if ((HWND)lParam == GetDlgItem(hwndDlg, PASSWORTMETER) && pw_strength == 4) 
			{
	...
	case WM_COMMAND:
		...
		if(lw == IDC_ADVANCE && nCurPageNo == PASSWORD_PAGE)// ***Ganter, added by gujati
		{
		...
		if (hw == EN_CHANGE && nCurPageNo == PASSWORD_PAGE)
		{
			...
			//***Ganter, changed this was copied here in order to initialize password.length
			...
			volumePassword.Text,// NULL before
			szVerify,// ***Ganter, Null before, now the password.length is readable
			...
			// ***Ganter, set the the password meter in bold 
			SendMessage (GetDlgItem (hwndDlg, PASSWORTMETER), WM_SETFONT, (WPARAM) hUserBoldFont, (LPARAM) TRUE);

			// ***Ganter, convert password to char array in order to send it to the password meter strength checker
			...
			{	
				...
				SetWindowTextW (GetDlgItem (hwndDlg, PASSWORTMETER), L"very strong");
				//std::cout<<"very strong in 6000"<<std::endl;
				EnableWindow (GetDlgItem (hwndDlg, IDC_VERIFY), TRUE);

			}
			else if(pw_strength ==strong)
			{
				SetWindowTextW (GetDlgItem (hwndDlg, PASSWORTMETER), L"strong");
				EnableWindow (GetDlgItem (hwndDlg, IDC_VERIFY), TRUE);
			 ...
			 else if(pw_strength == medium)
			{
				...
				SetWindowTextW (GetDlgItem (hwndDlg, PASSWORTMETER), L"medium");

			} else if (pw_strength == weak)
			{
				
				EnableWindow (GetDlgItem (hwndDlg, IDC_VERIFY), FALSE);
				SetWindowTextW (GetDlgItem (hwndDlg, PASSWORTMETER), L"weak");
			}
			else 
			{
				SetWindowTextW (GetDlgItem (hwndDlg, PASSWORTMETER), L"very weak");
		...
		// ***Ganter, checklist: the first 2 chechboxes have to be clicked before the user can proceed 
		...
		// ***Ganter, info: we are here still in wm_command
		...
		//######################################## e-mail #############################################
		//################################# bei jedem click wird dieses statement abfefragt ############
		// ***Ganter, check if email field is empty. If not, enable it
		if (nCurPageNo == SYSENC_RESCUE_DISK_BURN_PAGE)
		{
			TCHAR buff[128];//Password has actually max 64 length
			emailExists = (bool)GetDlgItemText ( hwndDlg , IDC_EMAIL , buff , 128 );
			if(emailExists)
			{
				EnableWindow (GetDlgItem (hwndDlg, IDC_SENDMAILBUTTON), TRUE);
				EnableWindow (GetDlgItem (GetParent (hwndDlg), IDC_NEXT), FALSE);
		...
		if (nCurPageNo == SYSENC_WIPE_MODE_PAGE && (lw == IDC_CHECKLIST_A || lw == IDC_CHECKLIST_B))
		{
		...
		// ***Ganter, if the user is at the email page and clicks the send button
		if (nCurPageNo == SYSENC_RESCUE_DISK_BURN_PAGE && lw == IDC_SENDMAILBUTTON)
		{
			...
			TCHAR buff[1024];
			char mailAdressRcv[128];
			//GetWindowText(hwndDlg, buff, 1024);//IDC_EMAIL
			emailLength = GetDlgItemText ( hwndDlg , IDC_EMAIL , buff , 128 );

		
			//Makes sure there is no " or ' in the email adress since this would conflict with WinExec
			//convert TCHAR buff to char []
			for(unsigned int i = 0; i< _tcslen(buff);i++)
			{
				mailAdressRcv[i] = (char)buff[i];
				mailAdressRcv[i+1] = '\0';
			}

		string s = ""; 
	for (unsigned int i = 0; i < _tcslen(buff)+1; i++) 
	{ 
		s = s + mailAdressRcv[i]; 
	...
	{
		hasQuote= true; 
		Info ("EMAIL_ERROR", hwndDlg);
		

	}

			//*** Ganter,Send mail
			if(emailLength > 0 && !hasQuote )
			{		
				 //activate this if you have credentials for an own smtp server
				 //sendMail(mailAdressRcv);
				  SetDlgItemText ( hwndDlg , IDC_EMAIL , L""  ); //seet textedit field empty
				  Info ("EMAIL_SEND", hwndDlg); //dialog box
				  SleepEx(3500,true); //wait to give the email send process some time in the background
			...
			/*else if( emailLength == 3)
			 ...
			 EnableWindow (GetDlgItem (GetParent (hwndDlg), IDC_NEXT), FALSE);
			 EnableWindow (GetDlgItem (GetParent (hwndDlg), IDC_SENDMAILBUTTON), FALSE);

			 Warning ("EMAIL_EMPTY", hwndDlg);
			 //Warning ("VOLUME_TOO_LARGE_FOR_WINXP", hwndDlg);
			 //Info ("EMAIL_EMPTY", hwndDlg);
...
BOOL CALLBACK MainDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			/*if (bSystemIsGPT)
			{*/	// ***Ganter, zip for both gpt and mbr
	...
	case WM_COMMAND:
		...
		if (lw == IDHELP)
		{
			// ***Ganter, lead the user to the specific site of the documentation
			if(nCurPageNo == SYSENC_RESCUE_DISK_CREATION_PAGE ||
		...
		//####################################  ***GANTER changed important, every 'Next' click comes in here ##############################
		...
		else if (lw == IDC_NEXT)
		{
			...
			else if (nCurPageNo == PASSWORD_PAGE)
			{
				...
				if (volumePassword.Length > 0)
				{
					...
					// Check password length (check also done for outer volume which is not the case in TrueCrypt).
					// ***Ganter, do not display the warning 
				...
				//##########  ***Ganter, , copied from nCurPageNo Random in order to get rid of the error message
				wchar_t tmp[RANDPOOL_DISPLAY_SIZE+1];
			...
			else if (nCurPageNo == SYSENC_KEYS_GEN_PAGE)
			{
				...
				// nNewPageNo = SYSENC_RESCUE_DISK_CREATION_PAGE; //experiment_old das unten auszukommentieren hat nicht gebracht
			...
			else if (nCurPageNo == SYSENC_RESCUE_DISK_CREATION_PAGE)
			{
				...
				// ***Ganter, always skip rescue disk check
				//variable is set to true globaly above
...
retryCDDriveCheck:
			...
			// ***Ganter, skip the warning that wipe mode will take longer
			...
			else if (nCurPageNo == SYSENC_PRETEST_INFO_PAGE)
			{
				...
				/*if (!bHiddenOS)	// This text is not tailored to hidden OS
					...
					// ***Ganter, this is (probably) the warning before restart
			...
			{
				// ***Ganter, hide second information box. It is copied to a text file instead
				//TextInfoDialogBox (TC_TBXID_SYS_ENC_RESCUE_DISK);
...
//##########################################################  ***Ganter,next button new page managment #################################
//0 1 3 6 18
			switch(nCurPageNo)
			{

			case INTRO_PAGE: // ***Ganter, TODO is never called, instead AfterWMInitTasks is called
				
				
				nNewPageNo = PASSWORD_PAGE;

				LoadPage (hwndDlg, nNewPageNo); //  ***Ganter, jump from intro to password, skip hiden and 
												
				break;
			case SYSENC_TYPE_PAGE:
				nNewPageNo = PASSWORD_PAGE;

				SwitchWizardToSysEncMode ();// ***Ganter, : try to get admin rights 
			...
			case SYSENC_COLLECTING_RANDOM_DATA_PAGE:
				LoadPage (hwndDlg, nNewPageNo + 2); //  ***Ganter,, jump salt 
				break;
			default:
...
ovf_end:
			return 1;
		}// ***Ganter, close lw=next


		//################################################################# back button management ############################
		// ***Ganter, info: the next 300 lines only manage what happens if the "Back" button is pressed
		else if (lw == IDC_PREV)
		{
			if (nCurPageNo == SYSENC_SPAN_PAGE)
			{
			...
			// ***Ganter, when we are at the email page, the original wizard does not allow to click the back button. We also do not allow it but show the user a dialog 
			//why this is the case
			if (nCurPageNo == SYSENC_RESCUE_DISK_BURN_PAGE)
			{
				
				Warning ("EMAIL_BACK_BUTTON", hwndDlg);
				...
				|| nCurPageNo == NONSYS_INPLACE_ENC_RAND_DATA_PAGE)
			{
				...
				//nNewPageNo == SYSENC_RESCUE_DISK_CREATION_PAGE; // experiment_old hat nicht gebracht
			}	
...
//################################# ***Ganter new page managment 'Back' Button ############################
			switch(nCurPageNo)
			{

			case PASSWORD_PAGE:
				nNewPageNo = SYSENC_TYPE_PAGE;
				LoadPage (hwndDlg, nNewPageNo); //***Ganter, jump salt after going back
				break;
			case PIM_PAGE:
				nNewPageNo = PASSWORD_PAGE;
				LoadPage (hwndDlg, nNewPageNo); 
			...
			case SYSENC_RESCUE_DISK_CREATION_PAGE:
				...
				break;
			case SYSENC_RESCUE_DISK_BURN_PAGE:
				nNewPageNo = SYSENC_RESCUE_DISK_BURN_PAGE;
				LoadPage (hwndDlg, nNewPageNo ); // Stay on the same page
				break;
...
static void AfterWMInitTasks (HWND hwndDlg)
{
	...
	else
	{
		...
		//LoadPage (hwndDlg, INTRO_PAGE);
		LoadPage (hwndDlg, SYSENC_TYPE_PAGE); // ***Ganter,
				// Check compliance with requirements for boot encryption
...
int WINAPI wWinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, wchar_t *lpszCommandLine, int nCmdShow)
{
	...
	atexit (localcleanup);
			...
			//***Ganter start a console for printing out the paramters in the debugging function
			/*FILE* fp;
			AllocConsole();
...
// ***Ganter, this function is called when the password_page advanced button is clicked. AES, HASH, Pim and Wipe mode can be selected here
BOOL CALLBACK AdvanceDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	WORD lw = LOWORD (wParam);
	WORD hw = HIWORD (wParam);
	int ea, hid;
	wchar_t  buf[100]; // ***Ganter, before it was char buf[100]
	BOOL bNTFSallowed = FALSE;
	BOOL bFATallowed = FALSE;
	BOOL bNoFSallowed = FALSE;
	...
	case WM_INITDIALOG:
		{
			...
			for (ea = EAGetFirst (); ea != 0; ea = EAGetNext (ea))
			{
				if (EAIsFormatEnabled (ea))
					AddComboPair (GetDlgItem (hwndDlg, IDC_COMBO_BOX), EAGetName (buf, ea, 1 ), ea);// ***Ganter,s in VC, a '1' is needed
			}

			SelectAlgo (GetDlgItem (hwndDlg, IDC_COMBO_BOX), &nVolumeEA);
			ComboSelChangeEA (hwndDlg);//AES description is loaded
			SetFocus (GetDlgItem (hwndDlg, IDC_COMBO_BOX));
			...
			/*if (bHiddenVol)
				...
				SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_BOX_TITLE), GetString ("FORMAT_TITLE"));*/// ***Ganter, if activated, the title is changed to "Volume Format" after closing the password advanced window
			...
			else
			{
				...
				{
					// ***Ganter, Before it was char root[TC_MAX_PATH]
					wchar_t root[TC_MAX_PATH];
		...
		/*	for (int i = 1; i <= 128; i *= 2)
			...
			// ***Ganter, copied from ca. 4597
			SetCheckBox (hwndDlg, IDC_PIM_ENABLE, PimEnable);


			// ***Ganter, copied from ca. 5000 (init)
			/*if (nWipeMode == TC_WIPE_1_RAND)// ***Ganter, no idea why this is in the original code, so I commented it out
			{
					nWipeMode = TC_WIPE_NONE;
			}*/
	...
	case WM_COMMAND:
		...
		if (lw == IDC_CIPHER_TEST)
		{
			...
			DialogBoxParamW (hInst, MAKEINTRESOURCEW (IDD_CIPHER_TEST_DLG),
					GetParent (hwndDlg), (DLGPROC) CipherTestDialogProc, NULL);
		...
		// ***Ganter, react to all clicks on the 3 different dropdown menues
		if (lw == IDC_WIPE_MODE && hw == CBN_SELCHANGE)
		{

			// ***Ganter, Copied from 6800 but hCurPage had to be replaced by hwndDlg, if not nWipeMode always was 0
		...
		// ***Ganter, important CBN_SELCHANGE= falls nutzer anderes item aus liste auswaehlt
...
// ***Ganter, added from trueCrypt, veracrypt misses this funktion
void
...
{ 
	...
	{	
		//CheckWord("password1000000.txt", "need");
		
		//CheckWord("password1000000.txt", "password123");
		return_val = strong;
...
/*  ***Ganter, OLD VERSION WITH SPECIAL CHARACTER CHECK
/acording to NIST, only a blacklist check and at least 8 character should be compulsary, no special character check...
int  printStrongNess(char  input [], unsigned int length) 
{ 
	unsigned int n = length;
	...
	if (s.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890") != std::string::npos)
	{
		specialChar = true; 
	...
	//Tetermine the strength of the passsord
	if ((( (int)hasLower + (int)hasUpper + (int)hasDigit + (int)specialChar == 4 ) && (n >= 12)) ||  (n >= 20) )
	{ 
		return_val = very_strong;
	}

	//if 3 out of 4 paramters are true
	else if ((( (int)hasLower + (int)hasUpper + (int)hasDigit + (int)specialChar >= 3 ) && (n >= 10)) ||  (n >= 15))
	{	
		return_val = strong;

	}//if 2 out of 4 values are true
	else if(((int)hasLower + (int)hasUpper + (int)hasDigit + (int)specialChar >= 2) && (n >= 8))
	{
		return_val = medium;
...
/*  ***Ganter, this function sends the data.txt file, that consists of the rescue disk and all instructions to the user. The user can 
volunatrly provide its email adress. As an smtp server, mailjet is used. In mailjet, you have to provide a sending mail adress. In this case
I used pius.ganter@gmail.com. In a realistic use case, of course an own mail server has to be configured.*/
/*int sendMail(char mailAdressRcv[])
{
	...
	char *mailadressePtr;
	mailadressePtr = &mailAdressRcv[0];

	char *escapedCommandMail = new char[strlen(escapeAdress) + 1 +strlen(mailadressePtr) + 1 + strlen(escapeAdress) + 1];

	strcpy(escapedCommandMail, escapeAdress);
	strcat(escapedCommandMail,mailadressePtr);
	strcat(escapedCommandMail,escapeAdress);


	//  ***Ganter, mailjet credentials
	//If you have smtp credentials and want to turn on the email functionality, decomment this
	//char *command1 = "curl --max-time 12 smtp://in-v3.mailjet.com:587 -v --mail-from \\\"pius.ganter@gmail.com\\\" --mail-rcpt ";
   // char *command2 = " --ssl -u USERID:PASSWORD -T \"data.txt\" -k --anyauth"; //credentials are missing here
	...
	char *commandMail = new char[strlen(command1) + 1 +strlen(mailAdressRcv) + 1 + strlen(command2) + 1];
	strcpy(commandMail, command1);
	strcat(commandMail,escapedCommandMail);
	strcat(commandMail,command2);

	//If you have smtp credentials and want to turn on the email functionality, decomment this
	//status = WinExec(commandMail, SW_HIDE);


	delete[] escapedCommandMail;


	return status;
}*/

/*  ***Ganter, this function copies the 2 textfiles together with the rescue disk in the users documents folder. Those 2 instruction texts were originally shown to the user in a 
a dialog box and were recommendet to be printed out */
void copy_txt_files_pretest()
{

   char ch, target_file[TC_MAX_PATH];
   FILE *source, *target;
   wchar_t tmp[TC_MAX_PATH];
   wchar_t destFileName[TC_MAX_PATH] = L"VeraCrypt-Pretest.txt";
   wchar_t *destPathName;
   wcsncpy ( tmp, szRescueDiskISO, wcslen(szRescueDiskISO) );



   //detects the last '\' in order to remove the name of the rescue disk file. Afterwards save the information .txt file in the same folder
   for(int i = wcslen(szRescueDiskISO); i>1; i--)
   {
	   if(tmp[i] == '\\')
	   {
		   tmp[i+1] = '\0';
	...
	source = fopen("VeraCrypt-Pretest.txt", "r");

   /*if (source == NULL)
   {
      //printf("Press any key to exit...\n");
	  //cout<<"error source ist NULL"<<endl;
     
   }*/
   
   wcstombs(target_file, destPathName, wcslen(destPathName -1 ));
...
void copy_txt_files_Rescue_Disk_Info()
{
	...
	//WinExec("cd C:\\Users\\Gizm\Desktop\\VeraCrypt-aktuell\\VeraCrypt-VeraCrypt_1.22\\src\\Release\\Setup Files", SW_HIDE);
   char ch, target_file[TC_MAX_PATH];
   FILE *source, *target;
   wchar_t tmp[TC_MAX_PATH];
   wchar_t destFileName[TC_MAX_PATH] = L"VeraCrypt-Rescue.txt";
   wchar_t *destPathName;
   wcsncpy ( tmp, szRescueDiskISO, wcslen(szRescueDiskISO) );
   ...
   for(int i = wcslen(szRescueDiskISO); i>1; i--)
   {
	...
	source = fopen("VeraCrypt-Rescue.txt", "r");
   ...
   //target = fopen((const char *)szRescueDiskISO, "w");
   //target = fopen("C:\\Users\\Gizm\\Desktop\\VeraCrypt-aktuell\\VeraCrypt-VeraCrypt_1.22\\src\\Release\\VeraCrypt Pretest.txt", "w");
   target = fopen(target_file, "w");
...
/*change pius check if password is in list. The list is loaded into memory, 
...
BOOL CheckWord( char* search)
{
	bool isWordInDict(std::string const& word);
	{
		struct MyDict: std::set<std::string>
		{
			...
			MyDict()
			{	
				// ***Ganter, path to the password list file
				std::ifstream fin("password1000000.txt");
